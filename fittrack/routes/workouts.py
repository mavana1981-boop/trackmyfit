...
@workouts_bp.route('/plans/<int:plan_id>/exercises/<int:pe_id>/suggest-weight', methods=['POST'])
@login_required
def suggest_weight(plan_id, pe_id):
    """Apply suggested weight increase to a plan exercise for next sessions."""
    plan = WorkoutPlan.query.filter_by(id=plan_id, user_id=current_user.id).first_or_404()
    pe = PlanExercise.query.filter_by(id=pe_id, plan_id=plan.id).first_or_404()
    new_weight = request.json.get('weight')
    if new_weight is not None:
        pe.suggested_weight = float(new_weight)
        db.session.commit()
        return jsonify({'ok': True, 'suggested_weight': pe.suggested_weight})
